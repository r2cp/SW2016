...
rng(663436);
  ...
  matdir = 'matlab/mat/gtM-const_vol_dtau-poos/';
...
%   dp = [dp_agg];
  ...
  for iseries = 1:size(dp,2)
    ...
    for t = i_first_poos:dnobs_ismpl;
      ...
      % Burnin Draws
      [tau,tau_f,sigma_dtau,sigma_eps,g_eps,g_dtau,scale_eps,ps] = ucsv_outlier_draw_const_vol_dtau(yn(1:t),n_burnin,sigma_eps,sigma_dtau,scale_eps,ps,g_eps_prior,g_dtau_prior,scl_eps_vec,ps_prior);
      
      % MCMC Draws to save at date t
      for i = 1:n_draws_save;
       [tau,tau_f,sigma_dtau,sigma_eps,g_eps,g_dtau,scale_eps,ps] = ucsv_outlier_draw_const_vol_dtau(yn(1:t),k_draws,sigma_eps,sigma_dtau,scale_eps,ps,g_eps_prior,g_dtau_prior,scl_eps_vec,ps_prior);
